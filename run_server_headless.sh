...
export PROTO_DEST_DIR="$(pwd)/target_dev"
mkdir -p "$PROTO_DEST_DIR"

./generate_protos.sh
mvn compile exec:java -Dbuild.dist.dir="$PROTO_DEST_DIR" -Dexec.mainClass="com.antigravity.App" -Dexec.args="--headless" -Dapp.data.dir="$(pwd)/../data"
